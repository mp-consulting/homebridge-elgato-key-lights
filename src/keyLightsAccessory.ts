...
const COLOR_TEMP_MIN = 143;
const COLOR_TEMP_MAX = 344;

/**
 * Clamp a value to the HomeKit color temperature range
 */
function clampColorTemperature(value: number): number {
  return Math.max(COLOR_TEMP_MIN, Math.min(COLOR_TEMP_MAX, value));
}
...
export class KeyLightsAccessory {
  ...
  onPropertyChanged(property: ('brightness' | 'on' | 'temperature'), value: number) {
    ...
    case 'on':
      ...
      break;
    case 'temperature':
      this.service.updateCharacteristic(this.platform.Characteristic.ColorTemperature, clampColorTemperature(value));
      break;
    case 'brightness':
